...
const state = {
  ...
  pendingAddonId: null,
  customerOrders: [], // طلبات العميل الحالي (My Account)
  customerOrdersUnsub: null,
  customerProfileDoc: null, // { uid, name, phone, tier } من كولكشن customers
  customerProfileDocUnsub: null,
  myAccountOpenOrderId: null,
};
...
function selectBranch(branchId) {
  ...
  if (typeof updateCustomerAuthUI === "function") updateCustomerAuthUI();
}
...
// معرّف العميل الحالي (سواء داخل بجوجل أو برقم/باسورد) — بيتخزن في كل أوردر
// عشان My Account تقدر تجيب طلبات العميل ده بس
function currentCustomerUid() {
  return (state.googleUser && state.googleUser.uid) || (state.customerProfile && state.customerProfile.uid) || null;
}
function currentCustomerName() {
  return (state.customerProfile && state.customerProfile.name) || (state.googleUser && state.googleUser.name) || "";
}
...
// ============================================================
// My Account — مستوى العميل، تتبع الطلبات، تسجيل دخول/حساب جديد
// ============================================================
function stopCustomerOrdersSync() {
  if (typeof state.customerOrdersUnsub === "function") state.customerOrdersUnsub();
  state.customerOrdersUnsub = null;
  state.customerOrders = [];
}
function startCustomerOrdersSync() {
  stopCustomerOrdersSync();
  const uid = currentCustomerUid();
  if (!uid || !state.branchId) return;
  state.customerOrdersUnsub = subscribeCustomerOrders(state.branchId, uid, (orders) => {
    state.customerOrders = orders;
    const modal = document.getElementById("myAccountModal");
    if (modal && modal.classList.contains("open")) renderMyAccountModal();
  });
}
function stopCustomerProfileSync() {
  if (typeof state.customerProfileDocUnsub === "function") state.customerProfileDocUnsub();
  state.customerProfileDocUnsub = null;
  state.customerProfileDoc = null;
}
function startCustomerProfileSync() {
  stopCustomerProfileSync();
  const uid = currentCustomerUid();
  if (!uid) return;
  state.customerProfileDocUnsub = subscribeCustomerProfileDoc(uid, (profile) => {
    state.customerProfileDoc = profile;
    const modal = document.getElementById("myAccountModal");
    if (modal && modal.classList.contains("open")) renderMyAccountModal();
    if (document.getElementById("productsGrid") && state.activeCategory !== ADDONS_TAB_ID) renderProducts();
  });
}

// ---------- المفضلة ----------
function isFavoriteProduct(productId) {
  return !!(state.customerProfileDoc && Array.isArray(state.customerProfileDoc.favorites) && state.customerProfileDoc.favorites.includes(productId));
}
async function handleFavoriteToggleClick(productId) {
  if (!isCustomerAuthed()) {
    showToast(t("favoritesLoginRequired"), "error");
    openMyAccountModal();
    return;
  }
  const uid = currentCustomerUid();
  const wasFav = isFavoriteProduct(productId);
  try {
    await toggleFavorite(uid, productId, !wasFav);
  } catch (err) {
    console.error(err);
    showToast(t("favoritesErrorToast"), "error");
  }
}
function findAnyProductById(productId) {
  return (
    ...
    state.birthday.find((p) => p.id === productId)
  );
}

// بتتنادى من auth.onAuthStateChanged (وأول ما الفرع يتحدد) — بتشغل/توقف
// اشتراكات الطلبات والبروفايل حسب حالة تسجيل الدخول
function updateCustomerAuthUI() {
  if (isCustomerAuthed()) {
    startCustomerOrdersSync();
    startCustomerProfileSync();
  } else {
    stopCustomerOrdersSync();
    stopCustomerProfileSync();
    state.myAccountOpenOrderId = null;
  }
  const modal = document.getElementById("myAccountModal");
  if (modal && modal.classList.contains("open")) renderMyAccountModal();
}

function openMyAccountModal(e) {
  closeBrowseMenu();
  renderMyAccountModal();
  openModalPop(document.getElementById("myAccountModal"), e && e.currentTarget);
}
document.getElementById("browseMyAccountBtn")?.addEventListener("click", (e) => openMyAccountModal(e));
document.getElementById("closeMyAccount")?.addEventListener("click", () => {
  closeModalPop(document.getElementById("myAccountModal"));
});
document.getElementById("myAccountOverlay")?.addEventListener("click", () => {
  closeModalPop(document.getElementById("myAccountModal"));
});
...
function renderMyAccountModal() {
  ...
  if (!isCustomerAuthed()) {
    body.innerHTML = myAccountLoginFormHtml();
    wireMyAccountLoginForm();
    return;
  }

  if (state.myAccountOpenOrderId) {
    const order = state.customerOrders.find((o) => o.id === state.myAccountOpenOrderId);
    if (order) {
      body.innerHTML = myAccountOrderDetailHtml(order);
      document.getElementById("myAccountBackBtn")?.addEventListener("click", () => {
        state.myAccountOpenOrderId = null;
        renderMyAccountModal();
      });
      document.getElementById("myAccountReorderDetailBtn")?.addEventListener("click", () => reorderOrder(order));
      return;
    }
    state.myAccountOpenOrderId = null;
  }

  body.innerHTML = myAccountDashboardHtml();
  document.querySelectorAll("#myAccountBody [data-order-id]").forEach((el) => {
    el.addEventListener("click", () => {
      state.myAccountOpenOrderId = el.getAttribute("data-order-id");
      renderMyAccountModal();
    });
  });
  document.querySelectorAll("#myAccountBody [data-reorder-id]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const order = state.customerOrders.find((o) => o.id === btn.dataset.reorderId);
      if (order) reorderOrder(order);
    });
  });
  document.querySelectorAll("#myAccountBody [data-fav-open]").forEach((el) => {
    el.addEventListener("click", () => {
      closeModalPop(document.getElementById("myAccountModal"));
      openProductModal(el.getAttribute("data-fav-open"), null, false);
    });
  });
  document.querySelectorAll("#myAccountBody [data-fav-toggle]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      handleFavoriteToggleClick(btn.dataset.favToggle);
    });
  });
...
function myAccountDashboardHtml() {
  const lang = getSiteLang();
  const profile = state.customerProfileDoc || {};
  const name = profile.name || currentCustomerName() || t("myAccountGuestName");
  const tier = profile.tier || DEFAULT_CUSTOMER_TIER;
  const orders = state.customerOrders || [];
  const ordersHtml = orders.length
    ? orders.map((o) => `
      <div class="myaccount-order-item">
        <div class="myaccount-order-item-top tap-fx" data-order-id="${o.id}">
          <span class="myaccount-order-code">#${o.code}</span>
          <span class="myaccount-order-status-pill">${escapeHtml(orderStatusLabel(o.status, lang))}</span>
        </div>
        <div class="myaccount-order-item-sub tap-fx" data-order-id="${o.id}">${new Date(o.createdAt).toLocaleString(lang === "ar" ? "ar-EG" : "en-US")} · ${o.total} ${t("currency")}</div>
        <button class="small-btn edit tap-fx myaccount-reorder-btn" data-reorder-id="${o.id}">${t("myAccountReorderBtn")}</button>
      </div>
    `).join("")
    : `<div class="myaccount-empty">${t("myAccountNoOrders")}</div>`;

  const favIds = (profile.favorites || []);
  const favProducts = favIds.map((id) => findAnyProductById(id)).filter(Boolean);
  const favHtml = favProducts.length
    ? favProducts.map((p) => `
      <div class="myaccount-fav-item">
        <img src="${p.image}" alt="${itemDisplayName(p)}" onerror="this.src='images/dialoglg.png'" class="myaccount-fav-img tap-fx" data-fav-open="${p.id}" />
        <div class="myaccount-fav-info tap-fx" data-fav-open="${p.id}">
          <div class="myaccount-fav-name">${escapeHtml(itemDisplayName(p))}</div>
          <div class="myaccount-fav-price">${p.price} ${t("currency")}</div>
        </div>
        <button class="fav-heart-btn active tap-fx" data-fav-toggle="${p.id}" aria-label="${t("favoritesToggle")}">❤️</button>
      </div>
    `).join("")
    : `<div class="myaccount-empty">${t("myAccountNoFavorites")}</div>`;

  return `
    <div class="myaccount-header">
      <div class="myaccount-avatar">👤</div>
      <div class="myaccount-header-text">
        <div class="myaccount-name">${escapeHtml(name)}</div>
        <div class="myaccount-tier-badge">${escapeHtml(customerTierLabel(tier, lang))}</div>
      </div>
    </div>
    <div class="myaccount-section-title">${t("myAccountOrdersTitle")}</div>
    <div class="myaccount-orders-list">${ordersHtml}</div>
    <div class="myaccount-section-title" style="margin-top:20px">${t("myAccountFavoritesTitle")}</div>
    <div class="myaccount-fav-list">${favHtml}</div>
    <button class="small-btn delete tap-fx" id="myAccountLogoutBtn" style="margin-top:14px;width:100%">${t("myAccountLogout")}</button>
  `;
}

// إعادة الطلب — بيضيف نفس الأصناف للسلة لو لسه متاحة، وبينبّه لو حاجة مبقتش موجودة
function reorderOrder(order) {
  const unavailableNames = [];
  let addedCount = 0;
  (order.items || []).forEach((it) => {
    const product = findAnyProductById(it.productId);
    ...
    state.cart.push({
      ...
      noPriceLabels: it.noPriceLabels || [],
      addonLabels: it.addonLabels || [],
  ...
  if (unavailableNames.length > 0) {
    showToast(t("reorderUnavailableToast", { names: unavailableNames.join("، ") }), "error");
  }
  if (addedCount > 0) {
    showToast(t("reorderAddedToast", { count: addedCount }), "success");
    closeModalPop(document.getElementById("myAccountModal"));
    openCartDrawer();
  }
}

function myAccountOrderDetailHtml(order) {
  const lang = getSiteLang();
  const pct = orderStatusPercent(order.status);
  const label = orderStatusLabel(order.status, lang);
  const circumference = Math.round(2 * Math.PI * 54 * 100) / 100;
  const offset = Math.round(circumference * (1 - pct / 100) * 100) / 100;
  const itemsHtml = (order.items || []).map((i) => `
    <div class="myaccount-order-detail-item">
      <span>${i.qty}× ${escapeHtml(i.name)}</span>
      <span>${i.lineTotal} ${t("currency")}</span>
    </div>
  `).join("");

  return `
    <button class="myaccount-back-btn tap-fx" id="myAccountBackBtn">${t("myAccountBack")}</button>
    <div class="myaccount-order-detail">
      <div class="myaccount-order-detail-code">#${order.code}</div>
      <div class="order-progress-circle-wrap">
        <svg class="order-progress-circle" viewBox="0 0 120 120">
          <circle class="order-progress-track" cx="60" cy="60" r="54"></circle>
          <circle class="order-progress-fill" cx="60" cy="60" r="54"
            style="stroke-dasharray:${circumference};stroke-dashoffset:${offset}"></circle>
        </svg>
        <div class="order-progress-percent">${pct}%</div>
      </div>
      <div class="order-progress-label">${escapeHtml(label)}</div>
      <div class="myaccount-order-detail-items">${itemsHtml}</div>
      <div class="myaccount-order-detail-total">${t("total")}: ${order.total} ${t("currency")}</div>
      <button class="glass-btn tap-fx" id="myAccountReorderDetailBtn" style="margin-top:14px;width:100%">${t("myAccountReorderBtn")}</button>
    </div>
  `;
}

function myAccountLoginFormHtml() {
  return `
    <div class="myaccount-auth-tabs">
      <button type="button" class="myaccount-auth-tab active tap-fx" data-tab="login">${t("myAccountLoginTab")}</button>
      <button type="button" class="myaccount-auth-tab tap-fx" data-tab="signup">${t("myAccountSignupTab")}</button>
    </div>
    <form class="myaccount-auth-form" id="myAccountLoginForm">
      <label>${t("phoneLabel")}</label>
      <input type="tel" id="maLoginPhone" placeholder="01xxxxxxxxx" inputmode="numeric" required />
      <label>${t("myAccountPasswordLabel")}</label>
      <input type="password" id="maLoginPassword" placeholder="${t("myAccountPasswordPlaceholder")}" required />
      <div class="myaccount-auth-err" id="maLoginErr"></div>
      <button type="submit" class="glass-btn tap-fx">${t("myAccountLoginBtn")}</button>
    </form>
    <form class="myaccount-auth-form" id="myAccountSignupForm" style="display:none">
      <label>${t("nameLabel")}</label>
      <input type="text" id="maSignupName" placeholder="${t("namePlaceholder")}" required />
      <label>${t("phoneLabel")}</label>
      <input type="tel" id="maSignupPhone" placeholder="01xxxxxxxxx" inputmode="numeric" required />
      <label>${t("myAccountPasswordLabel")}</label>
      <input type="password" id="maSignupPassword" placeholder="${t("myAccountPasswordPlaceholder")}" required minlength="6" />
      <div class="myaccount-auth-err" id="maSignupErr"></div>
      <button type="submit" class="glass-btn tap-fx">${t("myAccountSignupBtn")}</button>
    </form>
    <div class="myaccount-auth-or">${t("myAccountOr")}</div>
    <button type="button" class="browse-menu-item tap-fx" id="maGoogleBtn">
      <span>${t("browseGoogleSignIn")}</span>
    </button>
  `;
}

function wireMyAccountLoginForm() {
  const tabs = document.querySelectorAll(".myaccount-auth-tab");
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      tabs.forEach((x) => x.classList.remove("active"));
      tab.classList.add("active");
      const isLogin = tab.getAttribute("data-tab") === "login";
      document.getElementById("myAccountLoginForm").style.display = isLogin ? "" : "none";
      document.getElementById("myAccountSignupForm").style.display = isLogin ? "none" : "";
    });
  });

  document.getElementById("myAccountLoginForm")?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const errEl = document.getElementById("maLoginErr");
    errEl.textContent = "";
    const phone = document.getElementById("maLoginPhone").value.trim();
    const password = document.getElementById("maLoginPassword").value;
    if (!/^[0-9]{11}$/.test(phone)) { errEl.textContent = t("errPhone"); return; }
    if (!password) { errEl.textContent = t("myAccountErrPassword"); return; }
    try {
      await loginCustomerAccount({ phone, password });
      showToast(t("myAccountLoginSuccess"), "success");
      renderMyAccountModal();
    } catch (err) {
      console.error(err);
      errEl.textContent = t("myAccountErrLogin");
    }
  });

  document.getElementById("myAccountSignupForm")?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const errEl = document.getElementById("maSignupErr");
    errEl.textContent = "";
    const name = document.getElementById("maSignupName").value.trim();
    const phone = document.getElementById("maSignupPhone").value.trim();
    const password = document.getElementById("maSignupPassword").value;
    if (!name) { errEl.textContent = t("myAccountErrName"); return; }
    if (!/^[0-9]{11}$/.test(phone)) { errEl.textContent = t("errPhone"); return; }
    if (!password || password.length < 6) { errEl.textContent = t("myAccountErrPasswordLen"); return; }
    try {
      await signUpCustomerAccount({ name, phone, password });
      showToast(t("myAccountSignupSuccess"), "success");
      renderMyAccountModal();
    } catch (err) {
      console.error(err);
      errEl.textContent = err && err.code === "auth/email-already-in-use" ? t("myAccountErrPhoneUsed") : t("myAccountErrSignup");
    }
  });

  document.getElementById("maGoogleBtn")?.addEventListener("click", async () => {
    await googleSignIn();
    renderMyAccountModal();
  });
}
...
async function googleSignIn() {
  ...
  try {
    ...
    state.googleUser = { uid: user.uid, name: user.displayName || "", email: user.email || "", photo: user.photoURL || "" };
...
const BIRTHDAY_TAB_ID = "birthday_tab";
...
function renderCategories() {
  ...
  if (state.birthday && state.birthday.length > 0) {
    all.push({ id: BIRTHDAY_TAB_ID, name: t("categoryBirthday") });
  } else if (state.activeCategory === BIRTHDAY_TAB_ID) {
    state.activeCategory = "all";
  }
  ...
  bar.innerHTML = all
    .map(
      (c) =>
        `<button class="category-chip tap-fx ${c.id === OFFERS_TAB_ID ? "category-chip-offer" : ""} ${c.id === BIRTHDAY_TAB_ID ? "category-chip-birthday" : ""} ${c.id === ADDONS_TAB_ID ? "category-chip-addon" : ""} ${state.activeCategory === c.id ? "active" : ""}" data-cat="${c.id}">${c.id === "all" || c.id === OFFERS_TAB_ID || c.id === BIRTHDAY_TAB_ID || c.id === ADDONS_TAB_ID ? c.name : itemDisplayName(c)}</button>`
    )
    .join("");
...
function renderProducts() {
  ...
  const isBirthdayTab = state.activeCategory === BIRTHDAY_TAB_ID;
  ...
  const list = isOffersTab
    ? state.offers.filter((o) => !(o.expiresAt && o.expiresAt <= Date.now()))
    : isBirthdayTab
      ? state.birthday
      : state.activeCategory === "all"
        ? state.products
  ...
  if (list.length === 0) {
    grid.innerHTML = `<p style="grid-column:1/-1;text-align:center;color:var(--muted)">${isOffersTab ? t("noOffersInCategory") : isBirthdayTab ? t("noBirthdayInCategory") : t("noProductsInCategory")}</p>`;
    return;
  }
  ...
  grid.innerHTML = list
    .map((p) => {
      ...
      const favActive = isFavoriteProduct(p.id);
      return `
    <div class="product-card${!available ? " is-unavailable" : ""}">
      <div class="img-wrap">
        ...
        ${isBirthdayTab ? `<span class="offer-badge">${t("birthdayBadge")}</span>` : ""}
        ${isOffersTab && p.expiresAt ? `<span class="offer-countdown-badge" data-expires-at="${p.expiresAt}">--:--</span>` : ""}
        ${!available ? `<div class="sold-out-ribbon-wrap"><span class="sold-out-ribbon">${Array(6).fill(t("soldOutRibbon")).join("   •   ")}</span></div>` : ""}
        <button class="fav-heart-btn tap-fx${favActive ? " active" : ""}" data-fav-toggle="${p.id}" aria-label="${t("favoritesToggle")}">${favActive ? "❤️" : "🤍"}</button>
      </div>
      <div class="info">
        ...
        <div class="row">
          ...
          <button class="glass-btn tap-fx" style="padding:8px 16px" data-open="${p.id}" data-offer="${isOffersTab || isBirthdayTab ? "1" : "0"}" data-available="${available ? "1" : "0"}">${t("chooseBtn")}</button>
        </div>
      </div>
    </div>
  `;
  ...
  if (typeof tickOfferCountdowns === "function") tickOfferCountdowns();
}
...
// ============================================================
// عداد وقت العروض — بيعتمد على expiresAt المحفوظ في قاعدة البيانات (مش Timer محلي)
// عشان الـ Refresh ميرجعش يبدأ 24 ساعة من الأول تاني
function tickOfferCountdowns() {
  const badges = document.querySelectorAll(".offer-countdown-badge[data-expires-at]");
  if (!badges.length) return;
  let anyExpired = false;
  badges.forEach((el) => {
    const expiresAt = Number(el.getAttribute("data-expires-at"));
  ...
  if (anyExpired && state.activeCategory === OFFERS_TAB_ID) {
    renderCategories();
    renderProducts();
  }
}
...
function openProductModal(productId, triggerEl, isOffer) {
  const product =
    ...
    state.birthday.find((p) => p.id === productId);
  if (!product) return;
  const isOfferStyled = state.offers.some((o) => o.id === productId) || !!isOffer;

  state.modalProduct = product;
  state.modalIsOffer = isOfferStyled;
...
function changeCartQty(cartItemId, delta) {
  ...
  if (!item) return;
  if (delta > 0) {
    const product = state.products.find((p) => p.id === item.productId) || state.offers.find((p) => p.id === item.productId) || state.birthday.find((p) => p.id === item.productId);
