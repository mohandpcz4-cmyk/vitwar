...
async function googleSignIn() {
  ...
  try {
    ...
    updateGoogleAuthUI();
    startCustomerOrdersAndFavoritesSync();
...
async function googleSignOut() {
  ...
  updateGoogleAuthUI();
  stopCustomerOrdersAndFavoritesSync();
...
// ============================================================
// حسابي: المفضلة + طلباتي + تتبع الأوردر
// ============================================================
function customerAccountUid() {
  return (state.googleUser && state.googleUser.uid) || (state.customerProfile && state.customerProfile.uid) || null;
}

// نسخة عميل من orderProgressLabel() بتحترم لغة الموقع (عربي/إنجليزي)
// orderProgressLabel() الأصلية في store.js عربي بس (مستخدمة في لوحة الأدمن)
function customerOrderProgressLabel(progress) {
  return t("orderStage" + (progress || 0));
}

function requireAuthOrPrompt() {
  if (isCustomerAuthed() && customerAccountUid()) return true;
  showToast(t("browseGoogleSignIn"), "info");
  googleSignIn();
  return false;
}

function startCustomerOrdersAndFavoritesSync() {
  stopCustomerOrdersAndFavoritesSync();
  const uid = customerAccountUid();
  if (!uid) return;
  state.customerFavoritesUnsub = subscribeFavorites(uid, (favs) => {
    state.favorites = favs;
    renderProducts();
    renderFavoritesList();
  });
  state.customerOrdersUnsub = subscribeCustomerOrders(uid, (orders) => {
...
function stopCustomerOrdersAndFavoritesSync() {
  if (state.customerFavoritesUnsub) { try { state.customerFavoritesUnsub(); } catch (e) {} state.customerFavoritesUnsub = null; }
  if (state.customerOrdersUnsub) { try { state.customerOrdersUnsub(); } catch (e) {} state.customerOrdersUnsub = null; }
  state.favorites = [];
...
function updateMyOrdersBadge() {
  const badge = document.getElementById("browseMyOrdersBadge");
  if (!badge) return;
  const active = (state.customerOrders || []).filter((o) => !o.receivedByCustomer && !o.dismissedByCustomer).length;
...
async function toggleFavoriteProduct(product) {
  ...
  try {
    ...
    const nowFav = await toggleFavorite(uid, { id: product.id, name, image, price });
    // تحديث فوري للحالة محليًا (مش لازم ننتظر الـ subscription يلحق يرجّع)
    if (nowFav) {
      if (!isProductFavorite(product.id)) {
        state.favorites = [{ productId: product.id, name, image, price, addedAt: Date.now() }, ...(state.favorites || [])];
      }
    } else {
      state.favorites = (state.favorites || []).filter((f) => String(f.productId) !== String(product.id));
    }
    renderProducts();
    renderFavoritesList();
    showToast(nowFav ? t("favAddedToast") : t("favRemovedToast"), "success");
  } catch (err) {
    console.error(err);
    showToast(t("errOrderGeneric"), "error");
  }
}

function isProductFavorite(productId) {
  return (state.favorites || []).some((f) => String(f.productId) === String(productId));
}
...
function renderFavoritesList() {
  ...
  if (favs.length === 0) {
    body.innerHTML = `<p style="text-align:center;color:var(--muted)" data-i18n="favEmpty">${t("favEmpty")}</p>`;
    return;
  }
  body.innerHTML = favs.map((f) => `
    <div class="fav-card" data-fav-open="${f.productId}">
      <img src="${f.image || "images/dialoglg.png"}" alt="${escapeHtml(f.name || "")}" onerror="this.src='images/dialoglg.png'"/>
      <div class="fav-card-info">
        <div class="fav-card-name">${escapeHtml(f.name || "")}</div>
        ${f.price != null ? `<div class="fav-card-price">${f.price} ${t("currency")}</div>` : ""}
      </div>
      <button class="fav-card-remove tap-fx" data-fav-remove="${f.productId}">✕</button>
    </div>
  `).join("");

  body.querySelectorAll("[data-fav-open]").forEach((el) => {
    el.addEventListener("click", (e) => {
      if (e.target.closest("[data-fav-remove]")) return;
      const p = (state.products || []).find((x) => String(x.id) === el.dataset.favOpen);
      if (!p) { showToast(t("productUnavailableToast"), "error"); return; }
      closeModalPop(document.getElementById("favoritesModal"));
      openProductModal(p.id, null, false);
    });
  });
  body.querySelectorAll("[data-fav-remove]").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      e.stopPropagation();
      const uid = customerAccountUid();
      if (!uid) return;
      await toggleFavorite(uid, { id: btn.dataset.favRemove });
    });
  });
}

document.getElementById("browseFavoritesBtn")?.addEventListener("click", (e) => {
  closeBrowseMenu();
  if (!requireAuthOrPrompt()) return;
  renderFavoritesList();
  openModalPop(document.getElementById("favoritesModal"), e.currentTarget);
});
document.getElementById("closeFavoritesModal")?.addEventListener("click", () => closeModalPop(document.getElementById("favoritesModal")));
document.getElementById("favoritesModalOverlay")?.addEventListener("click", () => closeModalPop(document.getElementById("favoritesModal")));
...
function reorderFromOrder(order) {
  ...
  order.items.forEach((it) => {
    ...
    if (!product) return; // الصنف مبقاش موجود دلوقتي
    addedAny = true;
    state.cart.push({
  ...
  renderCartDrawer();
  renderCartCount();
  closeModalPop(document.getElementById("myOrdersModal"));
  if (addedAny) {
    showToast(t("reorderAddedToast"), "success");
    openCartDrawer();
  } else {
    showToast(t("reorderUnavailableToast"), "error");
  }
}

const MINI_RING_CIRC = 87.96; // 2 * π * 14
function miniRingSvg(pct) {
  return `
    <svg class="my-order-mini-ring-svg" viewBox="0 0 32 32">
      <circle class="ring-track" cx="16" cy="16" r="14"></circle>
      <circle class="ring-progress" cx="16" cy="16" r="14" data-target-offset="${MINI_RING_CIRC * (1 - Math.min(100, Math.max(0, pct)) / 100)}"></circle>
    </svg>
    <span class="my-order-mini-ring-value">${pct}%</span>`;
}

function renderMyOrdersList() {
  const body = document.getElementById("myOrdersBody");
  if (!body) return;
  const orders = (state.customerOrders || []).filter((o) => !o.dismissedByCustomer);
  if (orders.length === 0) {
    body.innerHTML = `<p style="text-align:center;color:var(--muted)">${t("myOrdersEmpty")}</p>`;
    return;
  }
  body.innerHTML = orders.map((o) => {
    const pct = o.receivedByCustomer ? 100 : (o.progress || 0);
    const stageLabel = o.receivedByCustomer ? t("orderReceivedDone") : customerOrderProgressLabel(o.progress);
    const time = new Date(o.createdAt).toLocaleString("ar-EG");
    const isDone = (o.progress || 0) >= 100;
    return `
    <div class="my-order-card${isDone ? " is-done" : ""}">
      ${isDone ? `<button class="my-order-dismiss-btn tap-fx" data-dismiss-order="${o.id}" aria-label="${t("dismissOrderLabel")}">✕</button>` : ""}
      <div class="my-order-card-top">
        <span class="my-order-code">#${o.code}</span>
        <span class="my-order-date">${time}</span>
      </div>
      <div class="my-order-card-top">
        <div class="my-order-stage" data-track-order="${o.id}" data-track-branch="${o.branchId}">
          <div class="my-order-mini-ring">${miniRingSvg(pct)}</div>
          <span>${stageLabel}</span>
        </div>
        <span class="my-order-total">${o.total} ${t("currency")}</span>
      </div>
      <div class="my-order-actions">
        <button class="glass-btn-secondary glass-btn tap-fx" data-reorder="${o.id}">${t("reorderBtn")}</button>
      </div>
    </div>`;
  }).join("");

  // بنسيب كل دائرة تتولد فاضية (0%) وبعد ما تتحط في الصفحة فعليًا
  // نديها النسبة الحقيقية عشان الأنيميشن يشتغل (لو حطينا القيمة
  // الصح من الأول، مفيش "تغيير" يخلي الـ transition يتفعل)
  requestAnimationFrame(() => {
    body.querySelectorAll(".ring-progress[data-target-offset]").forEach((circle) => {
      circle.style.strokeDashoffset = circle.dataset.targetOffset;
    });
  });

  body.querySelectorAll("[data-track-order]").forEach((el) => {
    el.addEventListener("click", (e) => {
      const order = orders.find((x) => x.id === el.dataset.trackOrder);
      if (order) openOrderTrackingModal(order.branchId, order.id, order.code, e.currentTarget);
    });
  });
  body.querySelectorAll("[data-reorder]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const order = orders.find((x) => x.id === btn.dataset.reorder);
      if (order) reorderFromOrder(order);
    });
  });
  body.querySelectorAll("[data-dismiss-order]").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      e.stopPropagation();
      const uid = customerAccountUid();
      if (!uid) return;
      try {
        await dismissCustomerOrder(uid, btn.dataset.dismissOrder);
        state.customerOrders = state.customerOrders.filter((o) => o.id !== btn.dataset.dismissOrder);
        renderMyOrdersList();
        updateMyOrdersBadge();
      } catch (err) {
        console.error(err);
...
document.getElementById("browseMyOrdersBtn")?.addEventListener("click", (e) => {
  closeBrowseMenu();
  if (!requireAuthOrPrompt()) return;
  renderMyOrdersList();
  openModalPop(document.getElementById("myOrdersModal"), e.currentTarget);
});
document.getElementById("closeMyOrdersModal")?.addEventListener("click", () => closeModalPop(document.getElementById("myOrdersModal")));
document.getElementById("myOrdersModalOverlay")?.addEventListener("click", () => closeModalPop(document.getElementById("myOrdersModal")));

// ---------- تتبع أوردر واحد (دائرة النسبة) ----------
function openOrderTrackingModal(branchId, orderId, code, triggerEl) {
  if (state.orderTrackingUnsub) { try { state.orderTrackingUnsub(); } catch (e) {} state.orderTrackingUnsub = null; }
  document.getElementById("orderTrackingCode").textContent = "#" + code;
  openModalPop(document.getElementById("orderTrackingModal"), triggerEl || null);
  state.orderTrackingUnsub = subscribeSingleOrder(branchId, orderId, (order) => {
    renderOrderTrackingState(branchId, order);
  });
}

const BIG_RING_CIRC = 326.7; // 2 * π * 52
function renderOrderTrackingState(branchId, order) {
  const pct = order.receivedByCustomer ? 100 : (order.progress || 0);
  const ringCircle = document.getElementById("orderTrackingRingCircle");
  const percentEl = document.getElementById("orderTrackingPercent");
  const labelEl = document.getElementById("orderTrackingStageLabel");
  const receivedBtn = document.getElementById("orderReceivedBtn");
  if (ringCircle) {
    const offset = BIG_RING_CIRC * (1 - Math.min(100, Math.max(0, pct)) / 100);
    ringCircle.style.strokeDashoffset = String(offset);
  }
  if (percentEl && percentEl.textContent !== pct + "%") {
    percentEl.textContent = pct + "%";
    percentEl.classList.remove("bump");
    // نعيد تشغيل الأنيميشن حتى لو نفس الكلاس كان متسجل قبل كده
    void percentEl.offsetWidth;
    percentEl.classList.add("bump");
  }
  if (labelEl) labelEl.textContent = order.receivedByCustomer ? t("orderReceivedDone") : customerOrderProgressLabel(order.progress);
  if (receivedBtn) {
    if ((order.progress || 0) >= 100 && !order.receivedByCustomer) {
      receivedBtn.style.display = "block";
      receivedBtn.onclick = async () => {
        receivedBtn.disabled = true;
        try {
          await markOrderReceivedByCustomer(branchId, order.id, customerAccountUid());
          showToast(t("orderReceivedThanks"), "success");
        } catch (err) {
          console.error(err);
...
document.getElementById("closeOrderTrackingModal")?.addEventListener("click", () => {
  closeModalPop(document.getElementById("orderTrackingModal"));
  if (state.orderTrackingUnsub) { try { state.orderTrackingUnsub(); } catch (e) {} state.orderTrackingUnsub = null; }
});
document.getElementById("orderTrackingModalOverlay")?.addEventListener("click", () => {
  closeModalPop(document.getElementById("orderTrackingModal"));
  if (state.orderTrackingUnsub) { try { state.orderTrackingUnsub(); } catch (e) {} state.orderTrackingUnsub = null; }
});
...
const BIRTHDAY_TAB_ID = "birthdays_tab";
...
function renderCategories() {
  ...
  if (state.birthdays && state.birthdays.length > 0) {
    all.push({ id: BIRTHDAY_TAB_ID, name: t("categoryBirthdays") });
  ...
  bar.innerHTML = all
    .map(
      (c) =>
        `<button class="category-chip tap-fx ${c.id === OFFERS_TAB_ID ? "category-chip-offer" : ""} ${c.id === ADDONS_TAB_ID ? "category-chip-addon" : ""} ${c.id === BIRTHDAY_TAB_ID ? "category-chip-birthday" : ""} ${state.activeCategory === c.id ? "active" : ""}" data-cat="${c.id}">${c.id === "all" || c.id === OFFERS_TAB_ID || c.id === ADDONS_TAB_ID || c.id === BIRTHDAY_TAB_ID ? c.name : itemDisplayName(c)}</button>`
    )
    .join("");
  bar.querySelectorAll(".category-chip").forEach((btn) => {
    btn.addEventListener("click", () => {
      const goingToBirthdays = btn.dataset.cat === BIRTHDAY_TAB_ID && state.activeCategory !== BIRTHDAY_TAB_ID;
      state.activeCategory = btn.dataset.cat;
      renderCategories();
      renderProducts();
      if (goingToBirthdays) showBirthdayStickerOverlay();
    });
  });
}
...
function renderProducts() {
  ...
  const list = isOffersTab
    ? state.offers.filter((o) => !o.expiresAt || o.expiresAt > Date.now()) // العروض اللي خلص وقتها تختفي عند العميل
    : isBirthdayTab
      ? state.birthdays
      : state.activeCategory === "all"
        ? state.products
  ...
  grid.innerHTML = list
    ...
    <div class="product-card${!available ? " is-unavailable" : ""}">
      <div class="img-wrap">
        <button class="product-fav-btn tap-fx${isProductFavorite(p.id) ? " is-fav" : ""}" data-fav-toggle="${p.id}">${isProductFavorite(p.id) ? "❤️" : "🤍"}</button>
  ...
  grid.querySelectorAll("[data-fav-toggle]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const p = list.find((x) => x.id === btn.dataset.favToggle);
      if (p) toggleFavoriteProduct(p);
    });
  });
  tickOfferCountdowns();
}

// عداد الوقت المتبقي على كل عرض (بيتحدث كل ثانية)، ولو عرض خلص وقته
// بيتشال من الشاشة تلقائي (من غير ما يتمسح من عند الأدمن)
function tickOfferCountdowns() {
  const badges = document.querySelectorAll(".offer-countdown-badge[data-expires-at]");
  if (badges.length === 0) return;
  let anyExpired = false;
  badges.forEach((el) => {
    const remaining = Number(el.dataset.expiresAt) - Date.now();
    if (remaining <= 0) {
      anyExpired = true;
      return;
    }
  ...
  if (anyExpired && state.activeCategory === OFFERS_TAB_ID) renderProducts();
}
setInterval(tickOfferCountdowns, 1000);

// ============================================================
// قسم "أعياد الميلاد": بلور الصفحة + استيكر عشوائي في النص لثواني
// ============================================================
const BIRTHDAY_STICKERS_COUNT = 6; // عدد ملفات الاستيكر في الفولدر (سمّيها sticker1.gif لحد sticker6.gif)
const BIRTHDAY_STICKER_DURATION_MS = 3200; // مدة ظهور الاستيكر قبل ما الشاشة ترجع عادي

function showBirthdayStickerOverlay() {
  const overlay = document.getElementById("birthdayStickerOverlay");
  const img = document.getElementById("birthdayStickerImg");
  const main = document.querySelector("main.container");
  if (!overlay || !img || !main) return;

  const idx = Math.floor(Math.random() * BIRTHDAY_STICKERS_COUNT) + 1;
  img.src = `images/birthday-stickers/sticker${idx}.gif`;
  img.onerror = function () { this.style.display = "none"; };
  img.onload = function () { this.style.display = ""; };

  main.classList.add("birthday-blurred");
  overlay.classList.add("show");

  setTimeout(() => {
    overlay.classList.remove("show");
    main.classList.remove("birthday-blurred");
  }, BIRTHDAY_STICKER_DURATION_MS);
}
...
function openProductModal(productId, triggerEl, isOffer) {
  const product = isOffer
    ...
    : (state.products.find((p) => p.id === productId) || state.offers.find((p) => p.id === productId) || state.birthdays.find((p) => p.id === productId));
  if (!product) return;
...
async function submitOrder() {
  ...
  const orderPayload = {
    ...
    fields,
    queued: isBusy,
    customerUid: customerAccountUid(),
  };
  ...
  if (isBusy) {
    ...
    openOrderTrackingModal(order.branchId, order.id, order.code, null);
  } else {
...
function showGlassNote(text, onClick) {
  const note = document.getElementById("glassNote");
  note.textContent = text;
  note.classList.add("show");
  note.style.cursor = onClick ? "pointer" : "";
