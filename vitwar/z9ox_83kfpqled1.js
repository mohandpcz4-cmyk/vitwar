...
function defaultBranchData() {
  return {
    ...
    deliveryEnabled: true, // فتح/قفل خدمة الديلفري من لوحة الأدمن
...
async function setBranchDeliveryEnabled(branchId, enabled) {
  await saveBranchData(branchId, { deliveryEnabled: !!enabled });
}
...
// ---------- الطلبات ----------
// مراحل تتبع الأوردر: 0% اترسل، 25% اتشاف، 50% بيتحضر، 75% خلص تحضير، 100% اتسلم
const ORDER_PROGRESS_STAGES = [
  { value: 0, label: "تم ارسال الاوردر" },
  { value: 25, label: "تم الاطلاع على الاوردر" },
  { value: 50, label: "جاري تحضير الاوردر" },
  { value: 75, label: "تم تحضير الاوردر" },
  { value: 100, label: "تم تسليم الاوردر" },
];
function orderProgressLabel(progress) {
  const stage = ORDER_PROGRESS_STAGES.find((s) => s.value === (progress || 0));
  return stage ? stage.label : ORDER_PROGRESS_STAGES[0].label;
}
...
async function addOrder(branchId, orderData) {
  ...
  if (firebaseReady) {
    ...
    fullOrder.id = ref.id;
    if (fullOrder.customerUid) {
      try {
        await db.collection("customers").doc(fullOrder.customerUid).collection("orders").doc(ref.id).set(fullOrder);
      } catch (err) { console.error("تعذر حفظ الأوردر في سجل العميل:", err); }
    }
  } else {
    ...
    if (fullOrder.customerUid) {
      const key = "vitwar_customer_orders_" + fullOrder.customerUid;
...
// الأدمن بيغيّر نسبة تقدّم الأوردر (0/25/50/75/100)
async function updateOrderProgress(branchId, orderId, progress) {
  const patch = { progress };
  if (firebaseReady) {
    const ref = db.collection("branches").doc(branchId).collection("orders").doc(orderId);
    const snap = await ref.get();
    await ref.update(patch);
    const data = snap.exists ? snap.data() : null;
    if (data && data.customerUid) {
      try {
        await db.collection("customers").doc(data.customerUid).collection("orders").doc(orderId).set(patch, { merge: true });
      } catch (err) { console.error("تعذر تحديث سجل العميل:", err); }
    }
  } else {
    const orders = localGetOrders(branchId);
    ...
    if (o) {
      ...
      localSaveOrders(branchId, orders);
      if (o.customerUid) {
        const key = "vitwar_customer_orders_" + o.customerUid;
        const list = JSON.parse(localStorage.getItem(key) || "[]");
        const co = list.find((x) => x.id === orderId);
        if (co) { co.progress = progress; localStorage.setItem(key, JSON.stringify(list)); }
      }
    }
  }
}

// العميل بيدوس "تم الاستلام" بعد ما الأوردر يوصل 100%
async function markOrderReceivedByCustomer(branchId, orderId, customerUid) {
  const patch = { receivedByCustomer: true };
  if (firebaseReady) {
    await db.collection("branches").doc(branchId).collection("orders").doc(orderId).update(patch);
    if (customerUid) {
      try {
        await db.collection("customers").doc(customerUid).collection("orders").doc(orderId).set(patch, { merge: true });
      } catch (err) { console.error(err); }
    }
  } else {
    const orders = localGetOrders(branchId);
    const o = orders.find((x) => x.id === orderId);
    if (o) { o.receivedByCustomer = true; localSaveOrders(branchId, orders); }
    if (customerUid) {
      const key = "vitwar_customer_orders_" + customerUid;
      const list = JSON.parse(localStorage.getItem(key) || "[]");
      const co = list.find((x) => x.id === orderId);
      if (co) { co.receivedByCustomer = true; localStorage.setItem(key, JSON.stringify(list)); }
    }
  }
}

// سجل أوردرات عميل معين (حسابي / طلباتي) — لحظي لو فايربيز شغالة
function subscribeCustomerOrders(uid, callback) {
  if (!uid) { callback([]); return () => {}; }
  if (firebaseReady) {
    return db.collection("customers").doc(uid).collection("orders")
      .orderBy("createdAt", "desc").limit(100)
      .onSnapshot((qs) => {
        const orders = [];
        qs.forEach((d) => orders.push(Object.assign({ id: d.id }, d.data())));
        callback(orders);
      }, (err) => { console.error("subscribeCustomerOrders:", err); callback([]); });
  }
  const key = "vitwar_customer_orders_" + uid;
  const read = () => JSON.parse(localStorage.getItem(key) || "[]");
  callback(read());
  const onStorage = (e) => { if (e.key === key) callback(read()); };
  window.addEventListener("storage", onStorage);
  const interval = setInterval(() => callback(read()), 2000);
  return () => { window.removeEventListener("storage", onStorage); clearInterval(interval); };
}

// تتبع أوردر واحد لحظيًا (لشاشة "فين الأوردر؟")
function subscribeSingleOrder(branchId, orderId, callback) {
  if (firebaseReady) {
    return db.collection("branches").doc(branchId).collection("orders").doc(orderId)
      .onSnapshot((snap) => {
        if (snap.exists) callback(Object.assign({ id: snap.id }, snap.data()));
      }, (err) => console.error("subscribeSingleOrder:", err));
  }
  const orders = localGetOrders(branchId);
  const o = orders.find((x) => x.id === orderId);
  if (o) callback(o);
...
// العميل بيدوس × على أوردر خلص (100%) عشان يشيله من قايمة "طلباتي"
// (الأوردر نفسه فضل موجود عند الفرع، بس اتشال من واجهة العميل بس)
async function dismissCustomerOrder(customerUid, orderId) {
  if (!customerUid) return;
  const patch = { dismissedByCustomer: true };
  if (firebaseReady) {
    await db.collection("customers").doc(customerUid).collection("orders").doc(orderId).set(patch, { merge: true });
  } else {
    const key = "vitwar_customer_orders_" + customerUid;
    const list = JSON.parse(localStorage.getItem(key) || "[]");
    const co = list.find((x) => x.id === orderId);
    if (co) { co.dismissedByCustomer = true; localStorage.setItem(key, JSON.stringify(list)); }
  }
}

// ---------- المفضلة ----------
function localFavoritesKey(uid) { return "vitwar_favorites_" + uid; }

// بيرجّع true لو اتضافت للمفضلة، أو false لو اتشالت (toggle)
async function toggleFavorite(uid, product) {
  if (!uid) return null;
  if (firebaseReady) {
    const ref = db.collection("customers").doc(uid).collection("favorites").doc(String(product.id));
    const snap = await ref.get();
    if (snap.exists) {
      await ref.delete();
      return false;
  ...
  const key = localFavoritesKey(uid);
  const list = JSON.parse(localStorage.getItem(key) || "[]");
  const idx = list.findIndex((f) => f.productId === product.id);
  if (idx >= 0) {
    list.splice(idx, 1);
    localStorage.setItem(key, JSON.stringify(list));
    return false;
  }
  list.unshift({ productId: product.id, name: product.name || "", image: product.image || "", price: product.price || null, addedAt: Date.now() });
  localStorage.setItem(key, JSON.stringify(list));
  return true;
}

function subscribeFavorites(uid, callback) {
  if (!uid) { callback([]); return () => {}; }
  if (firebaseReady) {
    return db.collection("customers").doc(uid).collection("favorites")
      .orderBy("addedAt", "desc")
      .onSnapshot((qs) => {
        const favs = [];
        qs.forEach((d) => favs.push(Object.assign({ id: d.id }, d.data())));
        callback(favs);
      }, (err) => { console.error("subscribeFavorites:", err); callback([]); });
  }
  callback(JSON.parse(localStorage.getItem(localFavoritesKey(uid)) || "[]"));
  const onStorage = (e) => { if (e.key === localFavoritesKey(uid)) callback(JSON.parse(localStorage.getItem(localFavoritesKey(uid)) || "[]")); };
  window.addEventListener("storage", onStorage);
  const interval = setInterval(() => callback(JSON.parse(localStorage.getItem(localFavoritesKey(uid)) || "[]")), 2000);
  return () => { window.removeEventListener("storage", onStorage); clearInterval(interval); };
}
