...
function showSupportForm() {
  ...
  teardownSupportSubscriptions();
  // لو العميل مسجل دخول، عبّي اسمه ورقمه تلقائيًا
  if (typeof state !== "undefined" && state.customerProfile) {
    const nameEl = document.getElementById("supportFieldName");
    const phoneEl = document.getElementById("supportFieldPhone");
    if (nameEl && !nameEl.value) nameEl.value = state.customerProfile.name || "";
    if (phoneEl && !phoneEl.value) phoneEl.value = state.customerProfile.phone || "";
  }
}
