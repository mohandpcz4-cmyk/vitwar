...
// ---------- الطلبات ----------
// ---------- حالات تتبع الطلب (تظهر للعميل كـ Progress Circle) ----------
const ORDER_STATUS_STEPS = ["received", "preparing", "prepared", "delivering", "delivered"];
const ORDER_STATUS_PERCENT = { received: 0, preparing: 25, prepared: 50, delivering: 75, delivered: 100 };
...
function orderStatusPercent(status) {
  return ORDER_STATUS_PERCENT[status] != null ? ORDER_STATUS_PERCENT[status] : 0;
}
function orderStatusLabel(status, lang) {
  const dict = ORDER_STATUS_LABELS[lang] || ORDER_STATUS_LABELS.ar;
  return dict[status] || dict.received;
}
...
// تحديث حالة الطلب (من لوحة الأدمن) — بيتحفظ في قاعدة البيانات فيبان فورًا عند العميل
async function updateOrderStatus(branchId, orderId, status) {
  if (firebaseReady) {
    await db.collection("branches").doc(branchId).collection("orders").doc(orderId).update({ status });
  } else {
    const orders = localGetOrders(branchId);
    const o = orders.find((x) => x.id === orderId);
...
// طلبات عميل معيّن بس (لصفحة My Account) — بترتب هنا في الكود بدل Firestore orderBy
// عشان نتجنب الحاجة لـ composite index على customerUid + createdAt
function subscribeCustomerOrders(branchId, customerUid, callback) {
  if (!customerUid) {
    callback([]);
    return () => {};
  }
  if (firebaseReady) {
    const unsub = db.collection("branches").doc(branchId).collection("orders")
  ...
  const orders = localGetOrders(branchId)
    .filter((o) => o.customerUid === customerUid)
...
async function signUpCustomerAccount({ name, phone, password }) {
  ...
  return { uid: cred.user.uid, name, phone, tier: DEFAULT_CUSTOMER_TIER };
}

// ---------- مستوى العميل (قابل للإدارة/التطوير من لوحة الأدمن لاحقًا) ----------
const DEFAULT_CUSTOMER_TIER = "regular";
const CUSTOMER_TIERS = {
  ar: { regular: "عميل عادي", silver: "عميل فضي", gold: "عميل ذهبي", vip: "VIP" },
  en: { regular: "Regular", silver: "Silver", gold: "Gold", vip: "VIP" },
};
function customerTierLabel(tier, lang) {
  const dict = CUSTOMER_TIERS[lang] || CUSTOMER_TIERS.ar;
  return dict[tier] || dict.regular;
}

// اشتراك لحظي في بيانات حساب العميل (الاسم/الرقم/المستوى) — بيتحدث فورًا
// لو الأدمن غيّر مستوى العميل من لوحة التحكم
// إضافة/إزالة منتج من المفضلة — بيتخزن في مستند العميل نفسه
async function toggleFavorite(uid, productId, isFav) {
...
function subscribeCustomerProfileDoc(uid, callback) {
  if (!firebaseReady || !db || !uid) {
    callback(null);
    return () => {};
  }
  return db.collection("customers").doc(uid).onSnapshot(
    (doc) => callback(doc.exists ? Object.assign({ uid }, doc.data()) : null),
    () => callback(null)
  );
}
