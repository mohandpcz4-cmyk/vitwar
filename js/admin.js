...
const state = {
  ...
  adminProductsCategoryFilter: "all",
};
...
document.getElementById("logoutBtn").addEventListener("click", async () => {
  ...
  if (typeof stopNewOrderLoopSound === "function") stopNewOrderLoopSound();
  if (typeof hideNewOrderPopup === "function") hideNewOrderPopup();
  orderPopupState.queue = [];
...
// ---------- تفعيل/تعطيل الديلفري ----------
function renderDeliveryToggle() {
  const btn = document.getElementById("deliveryToggleBtn");
  if (!btn) return;
  btn.classList.toggle("on", state.deliveryEnabled);
  btn.title = state.deliveryEnabled ? "الديلفري متاح — دوس عشان تقفله" : "الديلفري مقفول — دوس عشان تفتحه";
  const label = document.getElementById("deliveryToggleLabel");
  if (label) label.textContent = state.deliveryEnabled ? "متاح 🟢" : "مقفول 🔴";
}
document.getElementById("deliveryToggleBtn")?.addEventListener("click", async () => {
  const branchName = BRANCHES[state.branchId].name;
  const next = !state.deliveryEnabled;
  const ok = await siteConfirm(
    next
      ? `هتفتح خدمة الديلفري لفرع "${branchName}". متأكد؟`
      : `هتقفل خدمة الديلفري لفرع "${branchName}" — العميل هيشوف رسالة بديلة لو اختار توصيل. متأكد؟`,
    "تأكيد",
    "إلغاء"
  );
  if (!ok) return;
  state.deliveryEnabled = next;
  renderDeliveryToggle();
  await setBranchDeliveryEnabled(state.branchId, next);
  showToast(`تم ${next ? "فتح" : "قفل"} الديلفري ✅`, "success");
});
...
function loadAll() {
  subscribeBranch(state.branchId, (data) => {
    ...
    state.birthday = data.birthdayOffers || [];
...
document.getElementById("clearAllMessagesBtn")?.addEventListener("click", async () => {
  const ok = await siteConfirm("متأكد عايز تمسح كل الرسائل نهائي؟ مش هينفع ترجعها تاني.");
  if (!ok) return;
  const btn = document.getElementById("clearAllMessagesBtn");
  btn.disabled = true;
  try {
    await deleteAllBroadcastMessages(state.branchId);
    showToast("تم مسح كل الرسائل 🗑️", "success");
  } catch (err) {
    console.error(err);
    showToast("مقدرش أمسح الرسائل، حاول تاني", "error");
  } finally {
    btn.disabled = false;
  }
});
...
async function persistBranchMenu() {
  await saveBranchData(state.branchId, {
    ...
    offers: state.offers,
    birthdayOffers: state.birthday,
    addons: state.addons,
  });
}

// ================= نسخة احتياطية: تصدير / استيراد المنيو =================
function exportMenuBackup() {
  ...
  a.href = url;
  a.download = `vitwar-menu-backup-${state.branchId}-${dateStamp}.json`;
...
async function importMenuBackup(file) {
  const errEl = document.getElementById("backupError");
  const okEl = document.getElementById("backupSuccess");
  errEl.textContent = "";
  okEl.textContent = "";
  ...
  try {
    ...
    errEl.textContent = "الملف ده مش JSON صحيح، اتأكد إنه نفس الملف اللي نزلته من زرار التصدير.";
    return;
  }
  ...
  if (!hasValidShape) {
    errEl.textContent = "شكل الملف مش مطابق لنسخة Vitwar الاحتياطية (ناقصه تصنيفات/أصناف/عروض/إضافات).";
    return;
  }

  const summary = `هيتم استبدال: ${state.categories.length} تصنيف، ${state.products.length} صنف، ${state.offers.length} عرض، ${state.addons.length} إضافة\nبـ: ${data.categories.length} تصنيف، ${data.products.length} صنف، ${data.offers.length} عرض، ${data.addons.length} إضافة\n\nمتأكد عايز تكمل؟ الحاجة الحالية هتتمسح ومينفعش ترجعها إلا لو عندك نسخة احتياطية تانية.`;
  const ok = await siteConfirm(summary);
  if (!ok) return;

  state.categories = data.categories;
  state.products = data.products;
  state.offers = data.offers;
  state.birthday = Array.isArray(data.birthdayOffers) ? data.birthdayOffers : [];
  state.addons = data.addons;
  ...
  try {
    ...
    console.error(e);
    errEl.textContent = "حصل خطأ وقت الحفظ على Firebase، جرب تاني.";
    return;
  }
  ...
  loadBirthdayUI();
  loadAddonsLibraryUI();
  okEl.textContent = "تم استرجاع النسخة الاحتياطية بنجاح ✅";
}

document.getElementById("exportMenuBtn")?.addEventListener("click", exportMenuBackup);
...
function loadAddonsLibraryUI() {
  ...
  if (state.addons.length === 0) {
    tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;color:var(--muted)">لا يوجد إضافات بعد</td></tr>`;
  } else {
    tbody.innerHTML = state.addons
      ...
      <tr>
        ...
        <td>
          <div class="avail-toggle-row">
            <button type="button" class="avail-toggle tap-fx ${available ? "on" : ""}" data-toggle-addon-avail="${a.id}" title="${available ? "متاح — دوس عشان تقفلها" : "مقفولة — دوس عشان تفتحها"}">
      ...
      .join("");
    tbody.querySelectorAll("[data-toggle-addon-avail]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        const a = state.addons.find((x) => x.id === btn.dataset.toggleAddonAvail);
        if (!a) return;
        a.available = !(a.available !== false);
        await persistBranchMenu();
        loadAddonsLibraryUI();
      });
    });
...
document.getElementById("addonForm").addEventListener("submit", async (e) => {
  ...
  state.addons.push({ id, name, price, available: true });
  await persistBranchMenu();
...
// ================= الأصناف =================
function renderAdminProductsCategoryBar() {
  const bar = document.getElementById("adminProductsCategoryBar");
  if (!bar) return;
  const all = [{ id: "all", name: "الكل" }, ...state.categories];
  bar.innerHTML = all
    .map(
      (c) =>
        `<button class="category-chip tap-fx ${state.adminProductsCategoryFilter === c.id ? "active" : ""}" data-admin-cat="${c.id}">${c.name}</button>`
    )
    .join("");
  bar.querySelectorAll("[data-admin-cat]").forEach((btn) => {
    btn.addEventListener("click", () => {
      state.adminProductsCategoryFilter = btn.dataset.adminCat;
      loadProductsUI();
    });
  });
}
...
async function toggleProductAvailability(id) {
  ...
  if (!p) return;
  const effectivelyAvailable = p.available && !(typeof p.stock === "number" && p.stock <= 0);

  if (effectivelyAvailable) {
    // فتح → قفل: مباشرة من غير أي سؤال، والعميل هيشوف شريط "نفذت الكمية" فورًا
    p.available = false;
    await persistBranchMenu();
    loadProductsUI();
    return;
  ...
  // قفل → فتح: لازم يحدد الكمية المتاحة الأول
  const entered = await sitePrompt(`اكتب الكمية المتاحة من "${p.name}" (سيبها فاضية لو الكمية مش محدودة)`, {
    placeholder: "مثال: 10",
    value: typeof p.stock === "number" ? String(p.stock) : "",
    inputType: "number",
    okLabel: "فتح الصنف",
  });
  ...
  if (entered.trim() === "") {
    ...
    if (!Number.isFinite(n) || n < 0) {
      showToast("اكتب رقم صحيح للكمية", "error");
      return;
    }
    p.stock = Math.floor(n);
  }
  p.available = true;
  await persistBranchMenu();
  loadProductsUI();
...
function loadProductsUI() {
  if (state.adminProductsCategoryFilter !== "all" && !state.categories.some((c) => c.id === state.adminProductsCategoryFilter)) {
    state.adminProductsCategoryFilter = "all";
  }
  renderAdminProductsCategoryBar();
  const tbody = document.getElementById("productsTableBody");
  const list =
    state.adminProductsCategoryFilter === "all"
      ? state.products
      : state.products.filter((p) => p.category === state.adminProductsCategoryFilter);

  if (list.length === 0) {
    tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;color:var(--muted)">لا يوجد أصناف بعد</td></tr>`;
    return;
  }
  tbody.innerHTML = list
    .map((p) => {
      ...
      const priceLabel = p.variantGroups && p.variantGroups.length > 0
        ...
        : `${p.price} ج.م`;
      const stockLabel = typeof p.stock === "number" ? (p.stock === 0 ? "0 (خلص)" : p.stock) : "غير محدود";
      const effectivelyAvailable = p.available && !(typeof p.stock === "number" && p.stock <= 0);
      return `
      <tr>
        <td><img src="${p.image}" onerror="this.src='images/dialoglg.png'"/></td>
        ...
        <td>
          <div class="avail-toggle-row">
            <button type="button" class="avail-toggle tap-fx ${effectivelyAvailable ? "on" : ""}" data-toggle-avail="${p.id}" title="${effectivelyAvailable ? "متاح — دوس عشان تقفله" : "مقفول — دوس عشان تفتحه"}">
              <span class="avail-toggle-thumb"></span>
            </button>
          </div>
        </td>
  ...
  tbody.querySelectorAll("[data-toggle-avail]").forEach((btn) => {
    btn.addEventListener("click", () => toggleProductAvailability(btn.dataset.toggleAvail));
  });
...
// بتتأكد إن القيمة المكتوبة صالحة سواء كانت مسار محلي (images/xxx.jpg)
// أو رابط صورة مباشر (https://...). بترجع true/false.
function isValidImagePathOrLink(value) {
  ...
  // مسار محلي: مش لازم يكون فيه مسافات أو يبدأ بمسافة
  return value.trim().length > 0;
}

// بتحاول تحمّل الصورة فعليًا (سواء مسار محلي أو رابط خارجي) وترجع
// true لو اتحملت، false لو فشلت أو استغرقت وقت طويل (تايم آوت 8 ثواني).
// كده بنعرف قبل ما نحفظ الصنف لو الصورة فعلاً هتظهر عند العميل أو لأ.
...
// بتربط حقل مسار/لينك الصورة بفحص تلقائي: أول ما الأدمن يسيب الخانة
// (blur) بيتعمل تحميل تجريبي للصورة ويظهر نتيجة الفحص جنبها فورًا.
function attachImageLinkChecker(inputId, statusId) {
  ...
  input.addEventListener("blur", async () => {
    ...
    status.textContent = "";
    if (!value) return;
    if (!isValidImagePathOrLink(value)) {
      status.textContent = "⚠️ المسار/اللينك مش شكله صحيح";
      status.style.color = "#d9534f";
      return;
    }
    status.textContent = "⏳ جاري التأكد إن الصورة هتظهر...";
    status.style.color = "var(--muted, #888)";
    const ok = await checkImageLoads(value);
    // لو المستخدم غيّر القيمة أثناء الفحص، متعملش overwrite على نتيجة قديمة
    if (input.value.trim() !== value) return;
    if (ok) {
      status.textContent = "✅ تمام، الصورة اتحملت وهتظهر عند العميل";
      status.style.color = "#3aa76d";
    } else {
      status.textContent = "❌ الصورة دي مش هتظهر — تأكد إن اللينك بيرجّع صورة مباشرة مش صفحة، أو إن المسار المحلي صح";
      status.style.color = "#d9534f";
    }
  });
}
attachImageLinkChecker("pImagePath", "pImageCheckStatus");
attachImageLinkChecker("oImagePath", "oImageCheckStatus");
...
document.getElementById("productForm").addEventListener("submit", async (e) => {
  ...
  if (fileInput.files[0]) {
    ...
    if (!isValidImagePathOrLink(imagePath)) {
      errEl.textContent = "مسار/لينك الصورة مش صحيح — اكتب مسار زي images/item.jpg أو رابط يبدأ بـ https://";
      return;
    }
    errEl.textContent = "⏳ جاري التأكد إن الصورة هتظهر قبل الحفظ...";
    const loads = await checkImageLoads(imagePath);
    if (!loads) {
      errEl.textContent = "الصورة دي مش هتظهر عند العميل — تأكد من اللينك/المسار أو ارفع الصورة كملف بدل كده";
      return;
    }
    errEl.textContent = "";
    image = imagePath;
  ...
  const productData = {
    name, nameEn, category, price, description, available, isOffer, stock,
    image: image || "images/dialoglg.png",
...
function loadOffersUI() {
  ...
  if (state.offers.length === 0) {
    tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--muted)">لا يوجد عروض بعد</td></tr>`;
  } else {
    tbody.innerHTML = state.offers
      .map((o) => {
        ...
        return `
        <tr>
          <td><img src="${o.image}" onerror="this.src='images/dialoglg.png'"/></td>
          <td>${o.name}${o.nameEn ? `<br/><span style="color:var(--muted);font-size:.78rem">${o.nameEn}</span>` : ""}</td>
          <td>${priceLabel}</td>
          <td>${o.available === false ? "❌" : "✅"}</td>
          <td>${o.expiresAt ? formatRemainingTime(o.expiresAt) : "—"}</td>
          <td class="row-actions">
...
function startEditOffer(id) {
  ...
  document.getElementById("oDurationHours").value = "";
  document.getElementById("oExpiryInfo").textContent = o.expiresAt
    ? `الوقت المتبقي حاليًا: ${formatRemainingTime(o.expiresAt)} — سيب الخانة فاضية عشان يفضل زي ما هو، أو اكتب رقم جديد عشان تبدأ عداد جديد من دلوقتي`
    : "مفيش عداد وقت مضبوط للعرض ده حاليًا";
...
function resetOfferForm() {
  ...
  document.getElementById("oDurationHours").value = "";
  document.getElementById("oExpiryInfo").textContent = "";
...
document.getElementById("offerForm").addEventListener("submit", async (e) => {
  ...
  const durationHoursRaw = document.getElementById("oDurationHours").value.trim();
  ...
  const existingOffer = isEdit ? state.offers.find((o) => o.id === state.editingOfferId) : null;
  let image = isEdit ? existingOffer?.image : "";
  // مدة العرض: لو الأدمن كتب رقم، يبدأ عداد جديد من دلوقتي. لو سايبها فاضية وهو بيعدّل، العداد القديم (لو موجود) يفضل زي ما هو.
  let expiresAt = isEdit ? existingOffer?.expiresAt || null : null;
  if (durationHoursRaw !== "") {
    const hours = Number(durationHoursRaw);
  ...
  if (fileInput.files[0]) {
    ...
    if (!isValidImagePathOrLink(imagePath)) {
      errEl.textContent = "مسار/لينك الصورة مش صحيح — اكتب مسار زي images/offer.jpg أو رابط يبدأ بـ https://";
      return;
    }
    errEl.textContent = "⏳ جاري التأكد إن الصورة هتظهر قبل الحفظ...";
    const loads = await checkImageLoads(imagePath);
...
// ================= أعياد الميلاد =================
function loadBirthdayUI() {
  const tbody = document.getElementById("birthdayTableBody");
  if (!tbody) return;
  if (state.birthday.length === 0) {
    tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;color:var(--muted)">لا يوجد منتجات/عروض أعياد ميلاد بعد</td></tr>`;
  } else {
    tbody.innerHTML = state.birthday
      .map((o) => `
        <tr>
          <td><img src="${o.image}" onerror="this.src='images/dialoglg.png'"/></td>
          <td>${o.name}${o.nameEn ? `<br/><span style="color:var(--muted);font-size:.78rem">${o.nameEn}</span>` : ""}</td>
          ...
          <td>${o.available === false ? "❌" : "✅"}</td>
          <td class="row-actions">
            <button class="small-btn edit tap-fx" data-edit-birthday="${o.id}">تعديل</button>
            <button class="small-btn delete tap-fx" data-delete-birthday="${o.id}">حذف</button>
          </td>
        </tr>`)
      .join("");
  }

  tbody.querySelectorAll("[data-edit-birthday]").forEach((btn) => {
    btn.addEventListener("click", () => startEditBirthday(btn.dataset.editBirthday));
  });
  tbody.querySelectorAll("[data-delete-birthday]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const ok = await siteConfirm("متأكد عايز تحذف المنتج ده؟");
      if (!ok) return;
      state.birthday = state.birthday.filter((o) => o.id !== btn.dataset.deleteBirthday);
      await persistBranchMenu();
      loadBirthdayUI();
    });
  });
...
function startEditBirthday(id) {
  const o = state.birthday.find((x) => x.id === id);
  if (!o) return;
  state.editingBirthdayId = id;
  document.getElementById("birthdayFormTitle").textContent = "تعديل منتج/عرض عيد الميلاد";
  document.getElementById("bdId").value = id;
  document.getElementById("bdName").value = o.name;
  document.getElementById("bdNameEn").value = o.nameEn || "";
  document.getElementById("bdPrice").value = o.price;
  document.getElementById("bdDescription").value = o.description || "";
  document.getElementById("bdImagePath").value = o.image && !o.image.startsWith("data:") ? o.image : "";
  document.getElementById("bdAvailable").checked = o.available !== false;

  document.getElementById("cancelBirthdayEdit").style.display = "inline-flex";
  document.querySelector('.admin-tab[data-tab="birthday"]').click();
  window.scrollTo({ top: 0, behavior: "smooth" });
}

document.getElementById("cancelBirthdayEdit")?.addEventListener("click", resetBirthdayForm);
function resetBirthdayForm() {
  state.editingBirthdayId = null;
  document.getElementById("birthdayForm").reset();
  document.getElementById("bdId").value = "";
  document.getElementById("birthdayFormTitle").textContent = "إضافة منتج/عرض عيد ميلاد جديد";
  document.getElementById("cancelBirthdayEdit").style.display = "none";
  document.getElementById("birthdayError").textContent = "";
  document.getElementById("birthdaySuccess").textContent = "";
}

document.getElementById("birthdayForm")?.addEventListener("submit", async (e) => {
  e.preventDefault();
  const errEl = document.getElementById("birthdayError");
  const okEl = document.getElementById("birthdaySuccess");
  errEl.textContent = "";
  okEl.textContent = "";

  const name = document.getElementById("bdName").value.trim();
  const nameEn = document.getElementById("bdNameEn").value.trim();
  const price = Number(document.getElementById("bdPrice").value);
  const description = document.getElementById("bdDescription").value.trim();
  const available = document.getElementById("bdAvailable").checked;
  const imagePath = document.getElementById("bdImagePath").value.trim();
  const fileInput = document.getElementById("bdImageFile");

  if (!name || !price) {
    errEl.textContent = "الاسم والسعر مطلوبين";
    return;
  }

  const isEdit = !!state.editingBirthdayId;
  let image = isEdit ? state.birthday.find((o) => o.id === state.editingBirthdayId)?.image : "";
  if (fileInput.files[0]) {
    try {
      image = await fileToDataURL(fileInput.files[0]);
    } catch (err) {
    ...
    if (!isValidImagePathOrLink(imagePath)) {
      errEl.textContent = "مسار/لينك الصورة مش صحيح — اكتب مسار زي images/birthday.jpg أو رابط يبدأ بـ https://";
      return;
    }
    errEl.textContent = "⏳ جاري التأكد إن الصورة هتظهر قبل الحفظ...";
    const loads = await checkImageLoads(imagePath);
  ...
  if (isEdit) {
    const idx = state.birthday.findIndex((o) => o.id === state.editingBirthdayId);
    state.birthday[idx] = { ...state.birthday[idx], ...birthdayData };
    okEl.textContent = "تم تعديل المنتج بنجاح";
  } else {
    const id = "bd_" + Date.now().toString(36);
    state.birthday.push({ id, ...birthdayData });
    okEl.textContent = "تم إضافة المنتج بنجاح";
  }
...
function renderOrdersList() {
    ...
    .map((o) => {
      const itemsHtml = (o.items || [])
        .map((it) => {
          ...
          const addonsLine = (it.addonLabels || []).length
            ? `<div class="order-item-addons">✨ إضافات: ${it.addonLabels.map((a) => `${a.name} (+${a.price} ج.م)`).join("، ")}</div>`
            : "";
          return `<div class="order-item-line">${it.name}${options ? ` (${options})` : ""} × ${it.qty}${addonsLine}</div>`;
        })
        .join("");
      ...
      const statusOptionsHtml = ORDER_STATUS_STEPS.map(
        (s) => `<option value="${s}" ${s === currentStatus ? "selected" : ""}>${orderStatusLabel(s, "ar")} (${orderStatusPercent(s)}%)</option>`
      ).join("");
      ...
      <div class="order-card type-${o.type}">
        ...
        <div class="order-status-row">
          <label class="order-status-label" for="orderStatus_${o.id}">حالة الطلب:</label>
          <select class="order-status-select" id="orderStatus_${o.id}" data-order-status="${o.id}">
            ${statusOptionsHtml}
          </select>
        </div>
        <div class="order-top">
  ...
  list.querySelectorAll("[data-order-status]").forEach((sel) => {
    sel.addEventListener("change", async () => {
      const orderId = sel.dataset.orderStatus;
      const newStatus = sel.value;
      sel.disabled = true;
      try {
        await updateOrderStatus(state.branchId, orderId, newStatus);
        const o = state.orders.find((x) => x.id === orderId);
        if (o) o.status = newStatus;
        showToast("تم تحديث حالة الطلب ✅", "success");
      } catch (e) {
        console.error(e);
        showToast("حصل خطأ أثناء تحديث حالة الطلب", "error");
      } finally {
        sel.disabled = false;
      }
    });
...
function checkForNewOrders(orders) {
  orders.forEach((o) => {
    if (!o.seen && !state.playedSoundIds.has(o.id)) {
      ...
      enqueueNewOrderPopup(o);
    }
  });
  state.orders = orders;
  renderOrdersList();
  updateOrdersBadge();
  if (typeof renderStatsOrdersLog === "function") renderStatsOrdersLog();
}
...
function checkForNewChatMessages(chats) {
  ...
  if (state.activeChatId) {
    ...
    if (active) {
      ...
      // الأدمن فاتح الشات ده دلوقتي - أي رسالة جديدة من العميل تتعلّم "شوهدت" فورًا (تِكين أزرق)
      // بدل ما تفضل تِك واحد لحد ما يقفل الشات ويفتحه تاني
      if (active.unreadForAdmin) markChatSeenByAdmin(state.branchId, state.activeChatId);
    }
  }
}
...
function renderAdminChatMessages(msgs) {
  ...
  box.innerHTML = msgs
    .map((m) => {
      // من نحية الأدمن: رسايله هو تبقى "مني" (نفس ستايل customer اللي بيتحط على اليمين/جهة الإرسال)
      // ورسايل العميل تبقى "منه هو" (نفس ستايل admin اللي بيتحط على الجهة التانية) - عكس شاشة العميل بالظبط
      const cls = m.sender === "admin" ? "support-msg-customer" : m.sender === "customer" ? "support-msg-admin" : "support-msg-system";
      let ticks = "";
      if (m.sender === "admin") {
...
async function renderStatsPage() {
  const accountsEl = document.getElementById("statAccountsCount");
  const onlineEl = document.getElementById("statOnlineCount");
  const dailyEl = document.getElementById("statDailyRevenue");
  const totalEl = document.getElementById("statTotalRevenue");
  const errHint = document.getElementById("statsErrorHint");

  accountsEl.textContent = "…";
  dailyEl.textContent = "…";
  totalEl.textContent = "…";

  // عدد الحسابات المسجلة (كل الفروع)
  const accounts = await getTotalRegisteredAccounts();
  accountsEl.textContent = accounts === null ? "—" : accounts.toLocaleString("ar-EG");

  // فاتحين الموقع دلوقتي (لحظي - كل ما حد يفتح/يقفل الموقع بيتحدث تلقائي)
  if (statsPresenceUnsub) { try { statsPresenceUnsub(); } catch (e) {} }
  statsPresenceUnsub = subscribePresenceCount(state.branchId, (count) => {
    onlineEl.textContent = count === null ? "—" : count.toLocaleString("ar-EG");
  });

  // الإيرادات (يومي + إجمالي من كل الطلبات، مش بس آخر 200)
  const revenue = await getRevenueStats(state.branchId);
  dailyEl.textContent = formatEGP(revenue.daily);
  totalEl.textContent = formatEGP(revenue.total);

  const hasError = accounts === null || revenue.daily === null || revenue.total === null;
  errHint.style.display = hasError ? "block" : "none";

  renderStatsOrdersLog();
}

function renderStatsOrdersLog() {
  const box = document.getElementById("statsOrdersLog");
  if (!box) return;
  if (!state.orders || state.orders.length === 0) {
    box.innerHTML = `<p style="text-align:center;color:var(--muted)">لسه مفيش طلبات</p>`;
    return;
  }
  box.innerHTML = state.orders
    .map((o) => {
      ...
      <div class="stats-log-row">
        <span class="stats-log-code">#${o.code}</span>
        <span class="stats-log-time">${time}</span>
        <span class="stats-log-type">${ORDER_TYPE_LABELS[o.type] || o.type}</span>
        <span class="stats-log-total">${o.total} ج.م</span>
      </div>`;
    })
    .join("");
}
