...
async function importMenuBackup(file) {
  ...
  const summary = `هيتم استبدال: ${state.categories.length} تصنيف، ${state.products.length} صنف، ${state.offers.length} عرض، ${state.birthdays.length} عيد ميلاد، ${state.addons.length} إضافة\nبـ: ${data.categories.length} تصنيف، ${data.products.length} صنف، ${data.offers.length} عرض، ${(data.birthdays || []).length} عيد ميلاد، ${data.addons.length} إضافة\n\nمتأكد عايز تكمل؟ الحاجة الحالية هتتمسح ومينفعش ترجعها إلا لو عندك نسخة احتياطية تانية.`;
  ...
  state.products = data.products;
  state.offers = data.offers;
  state.birthdays = data.birthdays || []; // نسخ قديمة من غير القسم ده هتفضل شغالة
  state.addons = data.addons;
    ...
    tbody.innerHTML = state.offers
      .map((o) => {
        const priceLabel = o.variantGroups && o.variantGroups.length > 0
          ...
          : `${o.price} ج.م`;
        let timeLabel;
        if (!o.expiresAt) timeLabel = "دائم";
        else if (o.expiresAt <= Date.now()) timeLabel = `<span style="color:var(--danger,#e74c3c)">⛔ منتهي</span>`;
        else timeLabel = `⏱ ${Math.ceil((o.expiresAt - Date.now()) / 3600000)} ساعة متبقية`;
        return `
        <tr>
          ...
          <td>${priceLabel}<br/><span style="font-size:.75rem">${timeLabel}</span></td>
...
function startEditOffer(id) {
  ...
  document.getElementById("oDurationHours").value =
    o.expiresAt && o.expiresAt > Date.now() ? Math.max(1, Math.ceil((o.expiresAt - Date.now()) / 3600000)) : "";
...
document.getElementById("offerForm").addEventListener("submit", async (e) => {
  ...
  const durationHours = Number(document.getElementById("oDurationHours").value) || 0;
  ...
  const offerData = {
    ...
    variantGroups: cleanVariantGroups,
    noPriceGroups: cleanNoPriceGroups,
    expiresAt: durationHours ? Date.now() + durationHours * 3600 * 1000 : null,
  };
...
// ================= أعياد الميلاد =================
function loadBirthdaysUI() {
  const tbody = document.getElementById("birthdaysTableBody");
  if (!tbody) return;
  if (state.birthdays.length === 0) {
    tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;color:var(--muted)">لا يوجد أصناف أعياد ميلاد بعد</td></tr>`;
  } else {
    tbody.innerHTML = state.birthdays
      .map(
        (b) => `
        <tr>
          <td><img src="${b.image}" onerror="this.src='images/dialoglg.png'"/></td>
          <td>${b.name}${b.nameEn ? `<br/><span style="color:var(--muted);font-size:.78rem">${b.nameEn}</span>` : ""}</td>
          <td>${b.price} ج.م</td>
          <td class="row-actions">
            <button class="small-btn edit tap-fx" data-edit-birthday="${b.id}">تعديل</button>
            <button class="small-btn delete tap-fx" data-delete-birthday="${b.id}">حذف</button>
          </td>
        </tr>`
      )
      .join("");
  ...
  tbody.querySelectorAll("[data-delete-birthday]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const ok = await siteConfirm("متأكد عايز تحذف صنف عيد الميلاد ده؟");
      if (!ok) return;
      state.birthdays = state.birthdays.filter((b) => b.id !== btn.dataset.deleteBirthday);
      await persistBranchMenu();
      loadBirthdaysUI();
    });
  });
...
function startEditBirthday(id) {
  ...
  document.getElementById("birthdayFormTitle").textContent = "تعديل صنف عيد الميلاد";
  ...
  document.getElementById("bImagePath").value = b.image && !b.image.startsWith("data:") ? b.image : "";
  document.getElementById("bAvailable").checked = b.available !== false;
  document.getElementById("cancelBirthdayEdit").style.display = "inline-flex";
  document.querySelector('.admin-tab[data-tab="birthdays"]').click();
  window.scrollTo({ top: 0, behavior: "smooth" });
}
...
function resetBirthdayForm() {
  ...
  document.getElementById("birthdayFormTitle").textContent = "إضافة صنف عيد ميلاد جديد";
...
document.getElementById("birthdayForm")?.addEventListener("submit", async (e) => {
  ...
  const available = document.getElementById("bAvailable").checked;
  const imagePath = document.getElementById("bImagePath").value.trim();
  const fileInput = document.getElementById("bImageFile");

  if (!name || !price) {
    errEl.textContent = "اسم الصنف والسعر مطلوبين";
    return;
  }

  const isEdit = !!state.editingBirthdayId;
  let image = isEdit ? state.birthdays.find((b) => b.id === state.editingBirthdayId)?.image : "";
  if (fileInput.files[0]) {
    try {
      image = await fileToDataURL(fileInput.files[0]);
    } catch (err) {
    ...
    if (!isValidImagePathOrLink(imagePath)) {
      errEl.textContent = "مسار/لينك الصورة مش صحيح — اكتب مسار زي images/birthday1.jpg أو رابط يبدأ بـ https://";
      return;
    }
    errEl.textContent = "⏳ جاري التأكد إن الصورة هتظهر قبل الحفظ...";
    const loads = await checkImageLoads(imagePath);
  ...
  if (isEdit) {
    const idx = state.birthdays.findIndex((b) => b.id === state.editingBirthdayId);
    state.birthdays[idx] = { ...state.birthdays[idx], ...birthdayData };
    okEl.textContent = "تم تعديل الصنف بنجاح";
  } else {
    const id = "bd_" + Date.now().toString(36);
    state.birthdays.push({ id, ...birthdayData });
    okEl.textContent = "تم إضافة الصنف بنجاح";
  }
...
function renderOrdersList() {
    ...
    .map((o) => {
      ...
      <div class="order-card type-${o.type}">
        ...
        <div class="order-progress-row">
          <span class="order-progress-current-label">${orderProgressLabel(o.progress)}${o.receivedByCustomer ? " — ✅ العميل استلم" : ""}</span>
          <div class="order-progress-control">
            ${ORDER_PROGRESS_STAGES.map(
              (s) => `<button class="order-progress-btn tap-fx${(o.progress || 0) === s.value ? " active" : ""}" data-progress-order="${o.id}" data-progress-value="${s.value}" title="${s.label}">${s.value}%</button>`
            ).join("")}
          </div>
        </div>
        <div class="order-top">
  ...
  list.querySelectorAll("[data-del-order]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const ok = await sitePasswordConfirm("لازم تدخل الباسورد عشان تحذف الطلب ده", "sh@m");
      if (!ok) return;
      if (firebaseReady) {
  ...
  list.querySelectorAll("[data-progress-order]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const orderId = btn.dataset.progressOrder;
      const value = parseInt(btn.dataset.progressValue, 10);
      try {
        await updateOrderProgress(state.branchId, orderId, value);
        showToast("تم تحديث حالة الأوردر ✅", "success");
      } catch (err) {
        console.error(err);
        showToast("حصل خطأ في تحديث حالة الأوردر", "error");
      }
    });
  });
}
...
document.getElementById("clearOrdersBtn").addEventListener("click", async () => {
  const ok = await sitePasswordConfirm("لازم تدخل الباسورد عشان تمسح كل الطلبات", "sh@m");
  if (!ok) return;
