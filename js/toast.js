...
// تأكيد بباسورد (بديل الحذف العادي في حالات حساسة زي حذف الطلبات)
async function sitePasswordConfirm(message, correctPassword) {
  const val = await sitePrompt(message, { placeholder: "الباسورد", inputType: "password", okLabel: "تأكيد", cancelLabel: "إلغاء" });
  if (val === null) return false; // دوس إلغاء
  if (val !== correctPassword) {
    await siteAlert("الباسورد غلط ❌");
    return false;
  }
  return true;
}
