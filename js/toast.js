...
// ---------- لفة الصوت المتكررة عند أوردر جديد: بتفضل تتكرر لحد ما تدوس أي حتة، وبعدين تسكت ----------
let orderSoundLoopTimer = null;
let orderSoundStopBound = false;

function startOrderSoundLoop(type) {
  stopOrderSoundLoop();
  playOrderSound(type);
  orderSoundLoopTimer = setInterval(() => playOrderSound(type), 1800);
  bindStopOrderSoundOnAnyClick();
}

function stopOrderSoundLoop() {
  if (orderSoundLoopTimer) {
    clearInterval(orderSoundLoopTimer);
    orderSoundLoopTimer = null;
  }
}

function bindStopOrderSoundOnAnyClick() {
  if (orderSoundStopBound) return;
  orderSoundStopBound = true;
  document.addEventListener("click", stopOrderSoundLoop);
  document.addEventListener("touchstart", stopOrderSoundLoop);
}
