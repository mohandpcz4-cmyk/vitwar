...
function openWhatsAppOrder(order, branch) {
  ...
  const typeLabel = { cafe: "طلب كافيه", delivery: "طلب توصيل", pickup: "طلب استلام" }[order.type];
  let message = `${typeLabel} #${order.code} - ${branch.name} - Vitwar:%0A%0A`;
  (order.items || []).forEach((it) => {
    message += `- ${it.name}${it.optionsText ? ` (${it.optionsText})` : ""} × ${it.qty} = ${it.lineTotal} ج.م%0A`;
  });
  message += `%0Aالإجمالي: ${order.total} ج.م%0A%0A`;
  if (order.type === "cafe") {
    message += `الكافيه: ${order.fields.cafeName}%0Aطريقة الدفع: ${order.fields.payment}`;
  } else if (order.type === "delivery") {
    message += `العنوان: ${order.fields.address}%0Aطريقة الدفع: ${order.fields.payment}`;
  } else if (order.type === "pickup") {
    message += `وقت الاستلام المتوقع: ${order.fields.pickupTime}%0Aطريقة الدفع: ${order.fields.payment}`;
  }
